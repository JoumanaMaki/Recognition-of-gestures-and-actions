# -*- coding: utf-8 -*-
"""Recognition of gestures and actions.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1-rhK87HZD14nT8UpGtPBGlJgrumy14KH
...
image.shape, image.shape[0] * image.shape[1]*3 , image.shape[0] * image.shape[1]

image.shape[0] * image.shape[1]*3 # total nb of data
image.shape[0] * image.shape[1] #total nb of pixels

image_blob = cv2.dnn.blobFromImage(image, scalefactor=1.0/255, size=(image.shape[1] ,image.shape[0])) #dnn: deep neural network

image_blob.shape  #batch

type(image_blob)

"""### Load the pre-trained neural network"""

#caffe deep learning framework : another framework to work with instead of tensorflow
...
output = network.forward() #image is sent to the input layer in the nn and is going through all others layers

output.shape
#last 2 values are where the points in the images

position_width = output.shape[3]
position_height = output.shape[1]
...
image2 = cv2.imread('/content/drive/MyDrive/Computer Vision Masterclass/Images/player.jpg')
image_blob2 = cv2.dnn.blobFromImage(image2, scalefactor=1.0/255, size=(image2.shape[1] ,image2.shape[0]))
network.setInput(image_blob2)
output2 = network.forward()
position_width = output2.shape[3]
position_height = output2.shape[1]
num_points = 15
points = []
threshold = 0.1
for i in range(num_points):
...
def verify_arms_up(points):
  ...
  for i, point in enumerate(points): # enumerate to have the indexes
    #print(i, point)
    if i == 0:
      head = point[1]
    elif i == 4:
 ...
 # if right_wrist < head or left_wrist < head: # we use or if we want to test 1 hand is up
...
image = cv2.imread('/content/drive/MyDrive/Computer Vision Masterclass/Images/jump.jpg')
